...
Google Cloud DNS is a high-performance, resilient, and fully managed global DNS service that translates human-readable domain names (e.g., `app.company.com`) into IP addresses.

## Use case

Global domain resolution for public-facing web applications, service discovery within VPCs, and hybrid DNS resolution between on-premises datacenters and GCP.

## Example

`app.example.com` -> Cloud DNS Anycast -> `34.120.10.5` (Public Load Balancer IP)

## Architecture

### Complete End-to-End DNS Lookup Architecture
```text

[ Step 1: Request Initiated ]
   Client / VM / User Browser
   (e.g., Request for app.example.com)
            │
            │  1. Sends DNS Query
            ▼
[ Step 2: Traffic Entry Point ]
┌─────────────────────────────────────────────────────────┐
│  • Public Queries  ──► Global Anycast IP Network        │
│  • Private Queries ──► Internal Resolver (169.254.169.254)│
└───────────────────────────┬─────────────────────────────┘
                            │
                            │  2. Routes request to DNS Backend
                            ▼
[ Step 3: Google Infrastructure Engine ]
┌─────────────────────────────────────────────────────────┐
│               Cloud DNS Control Plane                   │
│   (Validates IAM permissions, VPC bindings & Routing)   │
└───────────────────────────┬─────────────────────────────┘
                            │
                            │  3. Matches Domain Container
                            ▼
[ Step 4: Zone Container ]
┌─────────────────────────────────────────────────────────┐
│                  Managed Zone                           │
│   • Public Zone (Internet-facing)                       │
│   • Private Zone (VPC-restricted)                       │
└───────────────────────────┬─────────────────────────────┘
                            │
                            │  4. Extracts exact mapped IP/Alias
                            ▼
[ Step 5: Final Response ]
┌─────────────────────────────────────────────────────────┐
│               Record Set Lookup                         │
│   • A Record (e.g., 34.120.10.5)                        │
│   • CNAME / TXT / MX Records                            │
└───────────────────────────┬─────────────────────────────┘
                            │
                            │  5. Returns IP back to Client
                            ▼
   Client establishes connection to target service

```

1. **Client / VM** sends the query (e.g., `lookup app.example.com`).
2. **Anycast IP / Internal DNS Proxy** receives the request at the entry point.
3. **Cloud DNS Control Plane** evaluates the request and routes it to the matching DNS container.
4. **Managed Zone (Public / Private)** opens the specific domain records.
5. **A / CNAME / TXT Records** extracts the matching answer and returns the IP address to the client.

This architecture illustrates **how a DNS query travels through Google Cloud DNS** to resolve a domain name into an IP address or record.
...
1. **Client / VM:**
The starting point. A user on the internet, a Compute Engine VM, or a GKE pod initiates a network request to reach a domain (e.g., `app.example.com`).

2. **Anycast IP / Internal DNS Proxy:**
The entry point for the DNS query:
* **For Public Requests:** Traffic hits Google’s **Global Anycast IP** network, routing the query to the geographically nearest Google edge location for minimal latency.
* **For Private VPC Requests:** The query hits the internal metadata DNS server (`169.254.169.254`) running inside the VPC.

3. **Cloud DNS Control Plane:**
The managed, highly available backend service that evaluates incoming requests, checks IAM/VPC permissions, and routes the query to the correct DNS zone based on the domain suffix.

5. **Managed Zone (Public / Private):**
The container where your specific domain rules live:
* **Public Managed Zone:** Handles external queries originating from the open internet.
* **Private Managed Zone:** Handles internal queries restricted to authorized VPC networks.


5. **A / CNAME / TXT Records:**
The ultimate destination. Cloud DNS fetches the requested DNS record (e.g., an **A record** returning an IP address, or a **CNAME** returning an alias) and sends the response back to the client.

---

## Important points

* **Global Scale:** Uses Google's global Anycast name servers for low latency and automatic scaling.
* **High Availability:** Guarantees 100% SLA uptime for authoritative name resolution.
* **No Server Management:** Completely serverless—no need to manage `bind` or core DNS instances.

---

## 30. Managed Zones: Public vs Private

Cloud DNS uses **Managed Zones** to containerize DNS record sets for a single domain name suffix.

## Use case

Public zones serve records to the open Internet, whereas Private zones host records accessible only from authorized VPC networks.

## Example

* **Public Zone:** `company.com` accessible by any user on the internet.
* **Private Zone:** `internal.corp` resolved only by internal GCE VMs or GKE pods.

## Architecture

```text
Public Query (Internet)  ──► Public Managed Zone   ──► External IP
VPC Internal Query       ──► Private Managed Zone  ──► Internal 10.x.x.x IP

```

## Difference table

| Feature | Public Managed Zone | Private Managed Zone |
| --- | --- | --- |
| **Visibility** | Public Internet | Restricted to authorized VPC networks |
| **Name Server Type** | Public Anycast Name Servers (`ns-cloud-a1.googledomains.com`) | Internal Metadata Server (`169.254.169.254`) |
| **Common Use** | External websites, public API endpoints | Microservices internal communication, database connection strings |

---

## 31. Hybrid DNS: Inbound & Outbound Server Policies

Server policies connect Google Cloud DNS with on-premises DNS infrastructure across Cloud VPN or Cloud Interconnect.

## Use case

Bi-directional name resolution: allow on-premises servers to resolve `.gcp.internal` domains, and GCP VMs to resolve `.corp.local` on-prem domains.

## Example

* **Inbound Policy:** On-prem app queries `db.gcp.internal` via an Inbound Forwarding Entry IP created inside the VPC.
* **Outbound Policy / Forwarding Zone:** GCP VM queries `active-directory.corp.local` and Cloud DNS forwards the request to on-prem DNS servers.

## Difference table

| Direction | Direction Flow | Mechanism Used |
| --- | --- | --- |
| **Inbound** | On-Premises ──► GCP VPC | Inbound Server Policy (allocates entry IP in VPC) |
| **Outbound** | GCP VPC ──► On-Premises | Forwarding Zone or Outbound Server Policy |

---

## 32. Advanced Routing Policies & Health Checks

Cloud DNS Routing Policies enable intelligent traffic steering and automatic failover directly at the DNS level without needing a load balancer.

## Use case

Multi-region disaster recovery, latency reduction, and traffic splitting for canary deployments.

## Routing Policy Types

1. **Weighted Round Robin (WRR):** Splitting traffic across different backends based on percentage weights (e.g., 80% to prod, 20% to new release).
2. **Geolocation Routing:** Directing users to the nearest regional endpoint based on the query's origin continent/country.
3. **Failover Routing:** Routing traffic to a primary regional endpoint; automatically shifting traffic to a secondary endpoint if health checks fail.

---

## 33. DNS Security (DNSSEC & Response Policies)

Security mechanisms built into Cloud DNS to protect against cache poisoning and enforce domain filtering.

## Important Points

* **DNSSEC:** Cryptographically signs DNS records to prevent spoofing and man-in-the-middle attacks.
* **Response Policies (RPZ):** Allows admins to set rules inside private VPCs to block malicious domains or override specific DNS lookups (e.g., sinkholing malware domains).

---

## Interview Nuggets

* **Public Zone vs Private Zone:** Public is globally queryable; Private is bound to specified VPC IDs.
* **GCP Internal Metadata IP:** `169.254.169.254` acts as the default local recursive resolver for Compute Engine VMs.
* **Hybrid Forwarding:** Inbound = On-Prem to GCP; Outbound = GCP to On-Prem.
* **Failover at DNS Level:** Cloud DNS Routing Policies combined with GCP Health Checks can execute multi-region DR failover without any proxy middleware.

...
**Retries happen on the original Subscription, not the Dead-Letter Topic.**

**1. The Retry Phase (On the Main Subscription)**

* When a subscriber pulls a message from the **Main Subscription**, the Ack Deadline timer starts.
* If the subscriber fails to process the message (returns a negative acknowledgment / `NACK` or the Ack Deadline expires without an `ACK`), Pub/Sub places that message back into the **same Main Subscription**.
* On the next pull request, the message is redelivered from that Main Subscription to an available worker.
* This redelivery cycle repeats inside the Main Subscription up to your configured **Maximum Delivery Attempts** (between 5 and 100).

**2. The Forwarding Phase (Moving to the Dead-Letter Topic)**

* When the delivery count reaches the limit (e.g., attempt 5 fails), Pub/Sub stops retrying the message on the Main Subscription.
* Pub/Sub automatically forwards the failing "poison-pill" message to the **[Dead-Letter Topic (DLQ)](https://github.com/SSSPRABHUDINESH/DevOps_Notes/blob/main/gcp/9.1%20Cloud-Pub-Sub.md#-what-is-a-dead-letter-topic)**.
* Pub/Sub then immediately issues an **ACK** on the Main Subscription to permanently remove the problematic message from the primary pipeline, preventing it from blocking other healthy messages.

**3. What Happens in the Dead-Letter Topic?**

* The Dead-Letter Topic does not automatically retry sending the message back to your main application.
* It simply acts as a storage holding area. To read those failed messages, you attach a separate subscription to the DLQ topic so engineers can inspect logs, fix the bug, or manually replay the payload.

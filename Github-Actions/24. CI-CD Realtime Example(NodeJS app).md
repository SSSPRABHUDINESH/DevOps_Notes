...
```yaml
name: Node.js CI/CD to GKE Pipeline
...
env:
  IMAGE_NAME: asia-south1-docker.pkg.dev/my-project/my-repo/myapp
  GKE_CLUSTER: my-gke-cluster
  GKE_ZONE: asia-south1-a

# Permissions required for GCP Workload Identity Federation (OIDC)
permissions:
  contents: read
  id-token: write
...
jobs:
  ci-build-test-scan-push:
    ...
    steps:
      ...
      # Step 7 — Dependency vulnerability scan (Trivy FS scan)
  ...
  cd-deploy-to-gke:
    ...
    steps:
      ...
      # Get GKE credentials to configure kubectl/Helm
      ...
      # Deploy via Helm (Rolling update)

# Technical Reference: OpenID Connect (OIDC) Integration in GitHub Actions (GCP, Terraform vault)
...
```
<img width="1232" height="437" alt="image" src="https://github.com/user-attachments/assets/ad1c123f-3f9e-42d9-9009-c21e0eca4769" />

Based on the architecture diagram and explanation provided in the video (1:34 - 2:45), the authentication flow follows this specific sequence:

1. **Initiation:** The *GitHub Actions* workflow initiates the process by requesting access to the *Google Cloud* environment.
2. **OIDC Token Presentation:** *GitHub Actions* presents its **OIDC (OpenID Connect) token** to the *Google Cloud Security Token Service (STS)*.
3. **Validation:** The *Security Token Service* receives the token and validates it against the pre-configured settings defined in the **Workload Identity Pool**.
4. **Credential Exchange:** Upon successful validation, the *Security Token Service* exchanges the *GitHub* token for a **short-lived access token** from *Google Cloud*.
5. **Authorization:** The *GitHub Action* receives this short-lived token, which is associated with a specific **Google Cloud Service Account**, allowing it to perform authorized tasks like running *Terraform* without static keys.
---

### How to configure Workload Identity Pool:

The pre-configured settings in the **Workload Identity Pool** act as the trust framework for incoming external identities. Based on the video (10:44 - 13:08), these settings primarily consist of:

*   **OIDC Issuer URL:** The trusted endpoint (e.g., *GitHub's* OIDC provider) that issues the tokens presented by the external entity.
*   **Attribute Mapping:** This defines how claims from the incoming token (e.g., from *GitHub*) are mapped to Google Cloud-specific attributes. Key mappings shown include:
    *   `google.subject` mapped to `assertion.sub`
    *   `attribute.actor` mapped to `assertion.actor`
    *   `attribute.repository` mapped to `assertion.repository`
*   **Attribute Conditions:** A security filter that ensures only specific entities can exchange tokens. The video specifically demonstrates restricting access by checking the repository name: `assertion.repository == 'your-org/your-repo'` (12:38 - 13:08).
...
* **`id-token: write`:** Grants the workflow runner permission to request a dynamic, short-lived OpenID Connect (OIDC) JSON Web Token (JWT) from GitHub’s identity provider. This token is what lets your workflow authenticate securely with GCP or Vault without static API keys.
* **`contents: read`:** Explicitly re-enables repository read access. As noted in the document, the moment you explicitly define *any* custom permission (like `id-token: write`), GitHub automatically revokes all other default token permissions. Without adding `contents: read`, action steps like `actions/checkout@v4` will fail because the runner loses the ability to clone your repository's code.

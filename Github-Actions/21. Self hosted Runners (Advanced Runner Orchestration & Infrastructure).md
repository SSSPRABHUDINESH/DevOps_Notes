...
## 3. Run it as a Background Service? To run permanently

If you simply execute 
```bash
./run.sh
```
- The runner will stop working the moment you close your terminal window.
- To keep it running permanently, register it as a system service on the VM:
```bash
sudo ./svc.sh install
sudo ./svc.sh start
```
